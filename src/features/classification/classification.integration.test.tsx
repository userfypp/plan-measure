...
function measuredState() {
  ...
  state = sessionReducer(state, { type: "ADD_CLASSIFICATION_DIMENSION", id: "trade", name: "Trade" });
  state = sessionReducer(state, { type: "ADD_CLASSIFICATION_VALUE", dimensionId: "trade", id: "electrical", name: "Electrical" });
  state = sessionReducer(state, { type: "ADD_CLASSIFICATION_VALUE", dimensionId: "trade", id: "plumbing", name: "Plumbing" });
  return state;
}
...
describe("classification domain integration", () => {
  it("assigns stable value IDs without changing calibration or geometry", () => {
    ...
    const after = sessionReducer(before, { type: "ASSIGN_CLASSIFICATION_VALUE", pageNumber: 1, measurementId: measurement.id, dimensionId: "trade", valueId: "electrical" });
  ...
  it("replaces the value from the same dimension and keeps archived historical IDs", () => {
    ...
    state = sessionReducer(state, { type: "ASSIGN_CLASSIFICATION_VALUE", pageNumber: 1, measurementId: "line-1", dimensionId: "trade", valueId: "electrical" });
    state = sessionReducer(state, { type: "ASSIGN_CLASSIFICATION_VALUE", pageNumber: 1, measurementId: "line-1", dimensionId: "trade", valueId: "plumbing" });
    expect(state.session!.pages[1]!.measurements[0]!.classificationValueIds).toEqual(["plumbing"]);

    state = sessionReducer(state, { type: "ARCHIVE_CLASSIFICATION_VALUE", dimensionId: "trade", id: "plumbing" });
    expect(state.session!.pages[1]!.measurements[0]!.classificationValueIds).toEqual(["plumbing"]);
    expect(state.session!.classificationCatalog.dimensions[0]!.values[1]!.archived).toBe(true);

    state = sessionReducer(state, { type: "RESTORE_CLASSIFICATION_VALUE", dimensionId: "trade", id: "plumbing" });
  ...
  it("rejects assigning an archived value", () => {
    ...
    const rejected = sessionReducer(state, { type: "ASSIGN_CLASSIFICATION_VALUE", pageNumber: 1, measurementId: "line-1", dimensionId: "trade", valueId: "electrical" });
