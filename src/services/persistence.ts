...
export async function replaceSavedSession(session: SessionV3, pdfBlob: Blob): Promise<void> {
  const database = await getDatabase();
...
export async function saveSessionMetadata(session: SessionV3): Promise<void> {
  const database = await getDatabase();
  const transaction = database.transaction(["sessions", "pdfs"], "readwrite");
